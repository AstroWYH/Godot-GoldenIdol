...
func update_data():
	...
	fill_left_story_container(self, story_data["sentence"])

# 主逻辑：将带占位符的句子渲染为可点击节点+文字
func fill_left_story_container(container: FlowContainer, text: String) -> void:
...
func create_wordentry(key: String) -> Control:
	...
	entry.set_label(entry_info)
	entry.toggle_label_visibility(false) # 默认不展示 玩家匹配时展示
	entry.word_type = GEnum.EWordPlace.LeftStory
	return entry
